...
//SQL문이 간단하게 생성된는 전략은 시퀀스 전략(오라클, H2에서만 지원)
//테이블 전략은 데이터베이스에 무관하게 범용적으로 사용할 수 있다는 장점이 있지만 테이블을 별도로 생성해야 하고 키 값을 자동으로 증가시키기 위해
//별도의 UPDATE 작업을 수행하는 등 성능상의 문제가 있다. 반면 시퀀스 전략은 별도의 테이블이 필요 없으며 등록 작업의 처리 속도도 빠르다.
